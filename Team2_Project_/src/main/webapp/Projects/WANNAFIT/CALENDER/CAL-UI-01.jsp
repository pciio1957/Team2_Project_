<%@page session="true" import="java.util.*" language="java"
	contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"
	import="jspexp.z01_database.*" import="jspexp.z02_vo.*"
	import="java.util.*"%>
<%
// JSP Calendar:--
...
<form>
	<table class="tab_all">
		<tr>
			<td class="td_head"><a
				href="cal.jsp?month=<%=currMonth%>&year=<%=currYear%>&action=0"><font
					size="1">Previous Month</font></a></td>
			<td class="td_head"><b><%=getDateName(cal.get(cal.MONTH)) + " " + cal.get(cal.YEAR)%></b></td>
			<td class="td_head"><a
				href="cal.jsp?month=<%=currMonth%>&year=<%=currYear%>&action=1"><font
					size="1">Next Month</font></a></td>
		</tr>
	</table>
	...
	<table class="td_body1">
		...
		<tr>
			...
			%>
			<td class="trd" onclick="goDetail(<%=dispDay%>)"
				bgcolor="<%=todayColor%>" width="<%=boxSize%>" align="left"
				height="<%=boxSize%>" valign="top" name="param" value="<%=dispDay%>"><%=dispDay%><button
					onclick="change()" class="btn">+</button> <br></td>
			<%
			count += 1;
			dispDay += 1;
			} else {
...
function change(){
    window.open("CAL-UI-02.jsp","popupView","width=350 height=350"); 
}
...
function goDetail(dispDay){
  ...
  //click한 td의 value	- > A0 뒤에 들어가ㅗ록 
 	...
 	location.href ="CAL-UI-03.jsp?param="+"A0"+n1;
	}
//
